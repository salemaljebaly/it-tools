...
- Skin tone variants are treated as the same base emoji (e.g. `raised_hands` vs `raised_hands::skin-tone-2`), so it won’t add multiple skin-tone reactions for the same emoji.
